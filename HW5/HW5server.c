...
/* The function for shell emulating.
 * It will process the input line, fork a child process to exec it and write to the temp file.
 */
void shell_service(void) {
    int i, c;
    int childStatus;
    ...
    while (forever) { /* actually, until EOF on connect socket */
        ...
        if(fp == NULL ){
            printf("Error on creating temp file\n");
            exit(-1);
        }
        dup2(1,2); //Redirects stderr to stdout, so certain error message can be sent to client
        ...
        if (childPid) {
            ...
            if(termPid == -1){
                ...
                if(WIFEXITED(childStatus)){ //check if the process exited normally and generate the response line
            ...
            /* Below seperate the input line into command and arguments.
             * This does not check whether the command is valid, 
             * as this error will be thrown later as a bad response line.
             */
            char* cmd;
...
/*
 * A helper function that reads the temp file and writes all characters into the socket until an EOF is met.
 */
void send_output(char * file_name) {
    FILE * f;
    int c;
    ...
    while (c != EOF) { //continue writing until EOF
